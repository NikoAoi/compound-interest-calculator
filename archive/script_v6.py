...
# --- 解决Matplotlib中文显示问题的函数 ---
def set_matplotlib_font():
    try:
        font_list = ['SimHei', 'Microsoft YaHei', 'PingFang SC', 'WenQuanYi Zen Hei', 'Noto Sans CJK SC']
...
class VisualCompoundInterestCalculator(ctk.CTk):
    def __init__(self):
        ...
        self.title("交互式复利计算器 (Interactive Compound Interest Calculator)")
        ...
        # --- UI组件部分 (与之前版本完全相同) ---
        # ... 为了简洁，这里省略了所有UI组件的创建代码，它们和上一版一样 ...
        title_label = ctk.CTkLabel(main_frame, text="复利的力量 (交互版)", font=self.title_font)
    ...
    def hover(self, event):
        if not self.plot_data or not event.inaxes == self.ax:
        ...
        # event.xdata 是图表上的x坐标 (例如 3.5个月)
        # 我们要把它转换成 self.plot_data 中的索引 (天数)
        idx = int(round(event.xdata * step))

        if 0 <= idx < len(self.plot_data):
            # 获取该点的精确数据
            day, amount = self.plot_data[idx]
            
            # 获取绘图时使用的坐标
            plot_x = day / step
            plot_y = amount
            
            # 更新 annotation 的位置和文本
    ...
    def setup_initial_plot(self):
        ...
        # 重新添加 annotation，因为 ax.clear() 会清除它
    ...
    def update_plot(self, *args):
        ...
        if num_points_to_plot > 2000: # 限制最多绘制的点数，防止卡顿
            step = len(self.plot_data) // 2000
        ...
        self.fig.patch.set_facecolor(colors["bg_color"])
        self.ax.set_facecolor(colors["bg_color"])
        self.ax.plot(x_data, y_data, marker='.', linestyle='-', color=colors["line_color"], markersize=2)
        self.ax.set_title("收益增长曲线", color=colors["text_color"])
